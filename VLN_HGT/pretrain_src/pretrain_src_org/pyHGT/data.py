...
class Graph():
    ...
    def get_types(self):
        return list(self.node_forward.keys())

### modification for ETP
def build_idx_to_node_id_mapping(graph, layer_data):
    ...
    # 遍历layer_data中的每个节点类型,以get_type为顺序
    for node_type in graph.get_types():
        # 遍历当前类型的每个节点ID
        for node_id, node_info in layer_data[node_type].items():
            # 保存当前索引到node id的映射
...
def get_FullGraphInfo(graph, feature_extractor=feature_OAG):
    ...
    layer_data  = defaultdict( #node_type
                        lambda: {} # {node_id: [ser, time]}
                    )
    
    ### build layer_data
    for node_type in graph.node_forward:
        for node_id in graph.node_forward[node_type]:
            layer_data[node_type][node_id] = [len(layer_data[node_type]), 0]
    # get the map from node id to tensor index
    idx2vpid_hgmap = build_idx_to_node_id_mapping(graph, layer_data)
