...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.HGT_encoder = GNN(conv_name = config.GNN_conv_name, in_dim = config.hidden_size, \
          n_hid = config.GNN_n_hid, n_heads = config.GNN_n_heads, n_layers = config.GNN_n_layers, dropout = config.GNN_dropout,\
          num_types = config.GNN_num_types, num_relations = config.GNN_num_relations)
        ...
        ## HGT
        # self.using_HGT = config.using_HGT
    ...
    def _aggregate_gmap_features(
        ...
        # print("shape of batch_gmap_img_fts 000", batch_gmap_img_fts.shape)
        
        batch_gmap_img_fts = torch.cat(
            [torch.zeros(batch_size, 1, batch_gmap_img_fts.size(2)).to(device), batch_gmap_img_fts], 
            dim=1
        )
        # print("shape of batch_gmap_img_fts 111", batch_gmap_img_fts.shape)
        return batch_gmap_img_fts
    
    def gmap_input_embedding(
        self, split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
        gmap_step_ids, gmap_pos_fts, gmap_lens, HGmaps_info
    ):
        ...
        ### HGT processing
        if 1:
            batch_HGT_embeds = self.HGT_processing(HGmaps_info, gmap_img_fts, gmap_vpids)  
            gmap_img_fts = batch_HGT_embeds
        ...
        # ### HGT processing
        # if self.using_HGT:
        #     batch_HGT_embeds = self.HGT_processing(HGmaps_info, gmap_embeds, gmap_vpids)  
        #     gmap_embeds = batch_HGT_embeds
        
        return gmap_embeds, gmap_masks
    

    ###HGT模块
    def HGT_processing(self, HGmaps_info, gmap_embeds,gmap_vpids):
        batch_size = len(gmap_vpids)
        batch_crosAtt_input = []
        # max_map_len = gmap_embeds[0].size(0) - 1
        # print("max_map_len", max_map_len)
        # for loop in each map in batch data
        for idx, (idx2vpid_gmap, idx2vpid_hgmap, node_type, edge_time,
                   edge_index, edge_type, _, _) in enumerate(HGmaps_info):
            #reorder the gmap_embeds for HGT processing
            #import pdb; pdb.set_trace()  # 设置断点
            
            map_len = len(gmap_vpids[idx]) ## length including the stop sign,
            single_gmap_embeds = gmap_embeds[idx][1:map_len]

            #把node embedding取出来
            # get new idx
            # print("idx2vpid_gmap", idx2vpid_gmap)
            # print("idx2vpid_hgmap", idx2vpid_hgmap)
            new_order4gmap_embs, new_order4hgmap_embs = self.gmapFT2HGmap_mapping(idx2vpid_gmap, idx2vpid_hgmap)
            # reorder gmap ft using new order 
            #HGT_input_embeds = single_gemap_embeds[new_order4gmap_embs][:map_len-1, :]
            HGT_input_embeds = single_gmap_embeds[new_order4gmap_embs]
            #get HGT embeddings
            #print("HGT_input_embeds shape befor HGT", HGT_input_embeds.shape)
            print(" HGT processing start ......................")
            hgmap_embeds = self.HGT_encoder.forward(HGT_input_embeds, node_type, \
                                   edge_time, edge_index, edge_type)
            #print("hgmap_embeds shape after HGT", hgmap_embeds.shape)
            #reorder HGT embeddings
            
            crosAtt_input = hgmap_embeds[new_order4hgmap_embs]
            # add stop sign and padding
            batch_crosAtt_input.append(crosAtt_input)
        
        batch_crosAtt_input = pad_tensors_wgrad(batch_crosAtt_input)
        # add a [stop] token at beginning
        # print("shape of batch_crosAtt_input 000", batch_crosAtt_input.shape)
        device = gmap_embeds[0].device
        batch_crosAtt_input = torch.cat(
            [torch.zeros(batch_size, 1, batch_crosAtt_input.size(2)).to(device), batch_crosAtt_input], 
            dim=1
        )
        # print("shape of batch_crosAtt_input 111", batch_crosAtt_input.shape)
        # batch_gmap_img_fts = torch.cat(
        #     [torch.zeros(batch_size, 1, batch_gmap_img_fts.size(2)).to(device), batch_gmap_img_fts], 
        #     dim=1
        # )

        return batch_crosAtt_input
    


    def gmapFT2HGmap_mapping(self, idx2vpid_gmap, idx2vpid_hgmap):
        new_order4gmap_embs = []
        new_order4hgmap_embs = []
        for idx, vpid_hg in idx2vpid_hgmap.items():
            for id_gmap, vpid_gmap in idx2vpid_gmap.items():
                if vpid_gmap == vpid_hg:
                    new_idx =  id_gmap
                    new_order4gmap_embs.append(new_idx)
                    break
        
        for idx, vpid_gmap in idx2vpid_gmap.items():
            for id_hg, vpid_hg in idx2vpid_hgmap.items():
                if vpid_hg == vpid_gmap:
                    new_idx =  id_hg
                    new_order4hgmap_embs.append(new_idx)
                    break

        
        new_order4gmap_embs = torch.tensor(new_order4gmap_embs, dtype=torch.long)
        new_order4hgmap_embs = torch.tensor(new_order4hgmap_embs, dtype=torch.long)
        
        
        # # Calculate the padding size
        # padding_size = max_map_len - len(new_order4gmap_embs)
        # # Pad the tensors
        # new_order4gmap_embs = F.pad(new_order4gmap_embs, (0, padding_size), value=max_map_len)
        # new_order4hgmap_embs = F.pad(new_order4hgmap_embs, (0, padding_size), value=max_map_len)

        return new_order4gmap_embs, new_order4hgmap_embs
    ...
    def forward(
        ...
        split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
        gmap_step_ids, gmap_pos_fts, gmap_lens, HGmaps_info, graph_sprels=None
...
class GlocalTextPathCMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, HGmap_info,
        ...
        if return_gmap_embeds:
            gmap_embeds = self.global_encoder(
                ...
                split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
                gmap_step_ids, gmap_pos_fts, gmap_lens, HGmap_info, graph_sprels=gmap_pair_dists,
            )
        else:
            gmap_embeds = None
    ...
    def forward_mlm(
        self, txt_ids, txt_lens, traj_view_img_fts, traj_view_dep_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, HGmaps_info
    ):
        # text embedding
        txt_token_type_ids = torch.zeros_like(txt_ids)
        txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
