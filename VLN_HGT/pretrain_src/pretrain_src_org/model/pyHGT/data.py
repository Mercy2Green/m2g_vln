    ...
    def add_node(self, node):
        ...
        if node['id'] not in nfl:
            ...
            nfl[node['id']] = ser
                # 添加节点特征
            if 'features' in node:
                self.node_feature[node['type']][node['id']] = node['features']
            else:
                # 如果没有特定的特征就初始化一个空字典
                self.node_feature[node['type']][node['id']] = {}
            return ser
        return nfl[node['id']]
...
### modification for ETP
def build_idx_to_node_id_mapping(layer_data):
    # 映射表：从tensor中的索引到node id
    tensor_idx_to_node_id = {}
    
    # 累加器，用于记录当前处理到的节点总数（即tensor中的索引）
    accumulator = 0
    
    # 遍历layer_data中的每个节点类型
    for node_type, nodes in layer_data.items():
        # 遍历当前类型的每个节点ID
        for node_id, node_info in nodes.items():
            # 保存当前索引到node id的映射
            tensor_idx_to_node_id[accumulator] = node_id
            # 累加器增加，准备处理下一个节点
            accumulator += 1
            
    return tensor_idx_to_node_id


def get_FullGraphInfo(graph, feature_extractor=feature_OAG):
    '''
    ...
    idx2vpid_hgmap = build_idx_to_node_id_mapping(layer_data)
    ...
    feature, times, _, _ = feature_extractor(layer_data, graph)

    # Assuming the to_torch function can handle the full graph's data,
    # just like it does for the sampled subgraph.
    _, node_type, edge_time, edge_index, edge_type, node_dict, edge_dict = to_torch(feature, times, edge_list, graph)

    return idx2vpid_hgmap, node_type, edge_time, edge_index, edge_type, node_dict, edge_dict
