...
## HGT
from pyHGT.data import Graph, get_FullGraphInfo
...
class ReverieTextPathData(object):
    ...
    def get_gmap_inputs(self, scan, path, cur_heading, cur_elevation):
        ...
        for t, vp in enumerate(path):
            #print("See! how dose vp look like:", vp)
...
class R2RTextPathData(ReverieTextPathData):
    ...
    def get_act_labels(self, end_vp, end_idx, item, gmap_vpids, traj_cand_vpids):
        ...
        return global_act_label, local_act_label
    ####### new for HGT
    def create_HGmap(self, scan, gmap_vpids,gmap_visited_masks, agent_heading):

        new_graph = Graph()  # 创建一个新的Graph实例
        visited_nodes = {gmap_vpids[i] for i, mask in enumerate(gmap_visited_masks) if mask == 1}  # 已访问的viewpoints
        candidate_nodes = {gmap_vpids[i] for i, mask in enumerate(gmap_visited_masks) if mask == 0 and i != 0}  # candidate viewpoints
        
        for vp in visited_nodes:
            #print("vpid in create_HGmap:", vp)
            new_visited_vp_info = {
                'type':1,
                'id':vp,
                'time':0
            }
            new_graph.add_node(new_visited_vp_info)  # 添加已访问节点
            
            # 获取当前viewpoint的位置
            curr_vp_position = self.graphs[scan].nodes[vp]['position']
            
            for connect_vp in self.scanvp_cands['%s_%s'%(scan, vp)].keys():
                if connect_vp in visited_nodes:
                    #若下一个node是visited vp
                    next_visited_vp_info = {
                        'type':1,
                        'id':connect_vp,
                        'time':0
                    }
                    #新节点的信息
                    new_graph.add_node(next_visited_vp_info)
                    #v-v edge的信息
                    ## first version of edge for v-v edge
                    relation_type = 'v_v_connected'
                    new_graph.add_edge(new_visited_vp_info, next_visited_vp_info, time=0, relation_type=relation_type, directed=False)
                    ###### dense direction relation representation
                    
                    # visited_vp_pos = self.graphs[scan].nodes[next_vp]['position']
                    # rel_heading, _, _ = calculate_vp_rel_pos_fts(curr_vp_position, visited_vp_pos, agent_heading)
                    # angle_interval = int((rel_heading + math.pi) / (2 * math.pi / 5))
                    # relation_type = str(angle_interval)
                    # ## (source node, target node)
                    # ### new_visited_vp_info == curr_vp
                    # new_graph.add_edge(next_visited_vp_info, new_visited_vp_info, time=0, relation_type=relation_type, directed=True)
                elif connect_vp in candidate_nodes:
                    #若下一个node是candidate vp
                    next_candidate_vp_info = {
                        'type':0,
                        'id':connect_vp,
                        'time':0
                    }
                    #新节点信息
                    new_graph.add_node(next_candidate_vp_info)
                    #v-c edge信息,计算相对方位信息
                    
                    cand_vp_pos = self.graphs[scan].nodes[connect_vp]['position']
                    rel_heading, _, _ = calculate_vp_rel_pos_fts(curr_vp_position, cand_vp_pos, agent_heading)
                    angle_interval = int((rel_heading + math.pi) / (2 * math.pi / 5))
                    relation_type = str(angle_interval)
                    new_graph.add_edge(new_visited_vp_info, next_candidate_vp_info, time=0, relation_type=relation_type, directed=True)
                
        return new_graph   
    
    def build_idx2vpid_gmap_mapping(self, gmap_vpids):
        """
        构建从gmap_vpids中的vpid到其索引的映射关系，假设gmap_img_fts是按照gmap_vpids的顺序构建的。

        参数:
        - gmap_vpids: 一组viewpoint ID。

        返回:
        - vpid_to_idx_mapping: 从vpid到其索引的映射字典。
        """
        idx2vpid_gmap = {}
        for idx, vp in enumerate(gmap_vpids[1:]):  # 从第二个vpid开始，因为假设gmap_img_fts从gmap_vpids的第二个元素开始构建
            idx2vpid_gmap[idx] = vp


        return idx2vpid_gmap
    ...
    def get_input(
        ...
        ####### new for HGT
        # print("#############################info in Dataset.py#########################")
        # print("#############   scan id is ", scan)
        HGmap = self.create_HGmap(scan, gmap_vpids, gmap_visited_masks, cur_heading)
        '''
        print("######################### start to debug!!!!!!!!!!")
        node_forward = HGmap.node_forward
        if not node_forward:
            print("node_forward is empty")
        for node_type, node_ids in node_forward.items():
            print(f"Target Type: {node_type}")
            for node_id, ser in node_ids.items():
                print(f"  Target ID: {node_id} -> Data: {node_id}")
        print("######################### debug end!!!!!!!!!!")
        '''

        ## HGT处理函数，获取tuple
        HGmap_input = get_FullGraphInfo(HGmap)
        ## get vpid to gmap idx的映射
        gmap_mapping = self.build_idx2vpid_gmap_mapping(gmap_vpids)
        HGmap_info = (gmap_mapping,) + HGmap_input
        
        # ### for debug
        
        # print("hgmap mapping in scan {}:".format(scan), HGmap_input[0])
        # print("gmap_mapping in scan {}:".format(scan), gmap_mapping)
        
        # if len(gmap_vpids)-1 != len(gmap_mapping):
        #     print("length of gmap", len(gmap_vpids))
        #     print("length of HGmap", len(gmap_mapping))
        #     print("gmap HGmap_info is not matching!!!")
        outs = {
            'instr_id': item['instr_id'],
            'instr_encoding': item['instr_encoding'][:self.max_txt_len],
