...
class Graph():
    def __init__(self):
        ...
        #self.node_feature = defaultdict(lambda: pd.DataFrame())
        node_types = ['object']
        self.node_feature = {node_type: pd.DataFrame() for node_type in node_types}
        '''
        ...
        self.edge_list = defaultdict( #target_type 
                            lambda: defaultdict( #source_type 
                                lambda: defaultdict( #relation_type 
                                    lambda: int)))
        relation_types = ['in', 'on', 'front', 'behind', 'left', 'right', 'connected', 'self']
        # Initialize edge_list with provided relation types
        for relation_type in relation_types:
            self.edge_list[relation_type] = defaultdict(lambda: defaultdict(lambda: int))        
    ...
    def add_node(self, node):
        ...
        Parameters:
        node (dict): A dictionary containing at least 'id', 'type', and optionally 'features'.
        """
        node_type = node['type']
        node_id = node['id']
        features = node

        # Check if node_id is already present
        node_index = self.node_forward[node_type].get(node_id)
        if node_index is None:
            # Assign new index to the node
            node_index = len(self.node_forward[node_type])
            self.node_forward[node_type][node_id] = node_index
            self.node_bacward[node_type].append(node)

            # Initialize features for the node
            # Ensure the DataFrame is large enough to include this new index
            if len(self.node_feature[node_type]) <= node_index:
                # Extend the DataFrame to accommodate new node
                self.node_feature[node_type] = self.node_feature[node_type].reindex(range(node_index + 1))

            # Set features for the node, default to NaN if not provided
            for feature_key, feature_value in features.items():
                print(f"feature key {feature_key} and feature value {type(feature_value)}")
                if feature_key not in self.node_feature[node_type]:
                    self.node_feature[node_type][feature_key] = np.nan
                
                self.node_feature[node_type].at[node_index, feature_key] = feature_value
        else:
            # Node exists, update features if any
            for feature_key, feature_value in features.items():
                self.node_feature[node_type].at[node_index, feature_key] = feature_value
    ...
    #####new for HGT
    def update_node_emb(self, node_type, node_id, node_emb):
        """
        给指定节点的node_emb特征赋值
        """
        if node_id in self.node_forward[node_type]:
            # 获取节点的序列号
            node_idx = self.node_forward[node_type][node_id]
            # 确保node_feature DataFrame有足够的行来存储新数据
            if node_type not in self.node_feature or len(self.node_feature[node_type]) <= node_idx:
                self.node_feature[node_type] = pd.DataFrame(index=range(len(self.node_backward[node_type])))
            # 更新或添加node_emb特征
            self.node_feature[node_type].at[node_idx, 'node_emb'] = node_emb
        else:
            print(f"Node {node_id} of type {node_type} does not exist in the graph.")



def get_HsgInfo(graph, feature_extractor=feature_OAG):
    '''
        Retrieve information from the entire graph.
    '''
    ...
    # Populate layer_data with all nodes across all types
    for node_type in graph.get_types():
        for node_id, node_info in enumerate(graph.node_bacward[node_type]):
            # Assuming each node_info is a dictionary that includes 'id' and possibly 'time'
            # Adjust according to your actual data structure.
            layer_data[node_type][node_info['id']] = [len(layer_data[node_type]), node_info.get('time', 0)]
    # get the map from node id to tensor index
    # Construct edge_list from the graph's edge_list
    ...
    # No need to change the way features are extracted
    obj_global_pos, obj_clip_fts = feature_extractor(layer_data, graph)
    # objwp_global_pos, obj_clip_fts, pos_fts, objwp_to_global, room_to_global, hgt2vpid = feature_extractor(layer_data, graph)

    # Assuming the to_torch function can handle the full graph's data,
    # just like it does for the sampled subgraph.
    # _, node_type, edge_time, edge_index, edge_type, _, _ = to_numpy(obj_clip_fts, edge_list, graph)

    return obj_clip_fts, obj_global_pos



def to_numpy(feature, edge_list, graph):
    '''
