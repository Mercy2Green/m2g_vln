...
def main(opts):
    ...
    lang_bert_path = "/home/lg1/lujia/VLN_HGT/bert-base-uncased"
    # tokenizer = AutoTokenizer.from_pretrained(model_config.lang_bert_name)
    tokenizer = AutoTokenizer.from_pretrained(lang_bert_path)
    ...
    if opts.checkpoint:
        ...
        if opts.init_pretrained == 'bert':
            ...
            tmp = AutoModel.from_pretrained(lang_bert_path)
    ...
    for step, (name, batch) in enumerate(meta_loader):
        ...
        print(f"Batch size for {name} in bytes: {batch_size}")
        print(f"Batch size for {name} in megabytes: {batch_size / (1024 ** 2):.2f} MB")
        ...
        if (step + 1) % opts.gradient_accumulation_steps == 0:
            ...
            if global_step % opts.valid_steps == 0:
                ...
                # val_unseen_loss = validate(model, val2_dataloaders, setname='_unseen')
                model_saver.save(model, global_step)

                # check whether early stop
                # early_stopper(val_unseen_loss)
                # val_unseen_loss = validate(model, val2_dataloaders, setname='_unseen')
...
def validate(model, val_dataloaders, setname=''):
    # print("######################## test whether model is in GPU#####################")
    ...
    val_unseen_loss = val_log.get('gloss')
    model.train()

    return val_unseen_loss
...
def validate_mlm(model, val_loader):
    ...
    n_word = 0
    st = time.time()
    # print length of val_loader
    # print(f"length of val_loader: {len(val_loader)}")
    # time_end_loop = st
    for i, batch in enumerate(val_loader):
        time_start_loop = time.time()
        # print(f"start to loop time {time.time() - st}")
        # print(f" time before every loop: {time_start_loop - time_end_loop}")
        # print(i)
        # print the time for model processing
        # time_1 = time.time()
        scores = model(batch, task='mlm', compute_loss=False)
        # time_2 = time.time()
        # print(f"model processing time: {time_2-time_1}")
        ...
        n_word += labels.numel()
        # print(f" time for other processing: {time.time()-time_2}")
