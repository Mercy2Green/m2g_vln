...
from vlnce_baselines.common.ops import extend_neg_masks, gen_seq_masks, pad_tensors_wgrad, pad_tensors_2dim
from vlnce_baselines.models.pyHGT.model import GNN
...
class RoomFeatureFilter(nn.Module):
    def __init__(self, feature_dim=768):
        ...
        self.clip_proj = nn.Sequential(
                        ...
                        BertLayerNorm(feature_dim, eps=1e-12))
    ...
    def forward(self, room_fts, text_embeds, assigned_room_idxs, batch_room_lens):
        ...
        txt_fts = self.text_proj(text_embeds).permute(0, 2, 1)  # [B, D, L]
        ...
        batch_rooms_fts_pre_weight, _ = (torch.matmul(batch_scan_fts, txt_fts.unsqueeze(1))).max(dim=-1)
        ...
        if batch_room_clips.isnan().any():
            print(f"batch_room_clips has nan value")
            print(f"batch_room_clips {batch_room_clips}")
...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.HGT_encoder = GNN(conv_name = config.GNN_conv_name, in_dim = 768, \
          ...
          num_types = config.GNN_num_types, num_relations=config.num_relations)
        # room filter
    ...
    def batch_HGTprocessing(self, batch_obj_embeds, batch_room_embeds, batch_graph_info_list, batch_graph_info_dict):
        ...
        for idx in range(batch_size):
            ...
            # print data type of room embeds and object embeds
            # print(f"obj_embeds.dtype {obj_embeds.dtype} and room_embeds.dtype {room_embeds.dtype}")
        ...
        # print the type of alld batch data
        # print(f"batch_node_embeds.dtype {batch_node_embeds.dtype} and batch_node_type.dtype {batch_node_type.dtype}")
        # print(f"batch_edge_time.dtype {batch_edge_time.dtype} and batch_edge_index.dtype {batch_edge_index.dtype}")
        # print(f"batch_edge_type.dtype {batch_edge_type.dtype}")
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_navigation(
        ...
        room_fts = F.normalize(room_fts, p=2, dim=-1)
        # global branch
        gmap_embeds = gmap_img_fts + \
        ...
        if self.global_encoder.sprel_linear is not None:
            ...
            graph_sprels = None
        # object nodes
        obj_pos_embeds = self.global_encoder.hgs_pos_fts_encoder(obj_pos_fts)
        obj_clip_fts = self.global_encoder.obj_emb_proj(obj_clip_fts)
        batch_obj_embeds = obj_clip_fts + obj_pos_embeds
        # room nodes
        room_embeds, room_masks = self.global_encoder.room_feature_filter(room_fts, txt_embeds, assigned_room_idxs,room_lens)
        room_pos_embeds = self.global_encoder.hgs_pos_fts_encoder(room_pos) 

        batch_room_embeds = room_embeds + room_pos_embeds
        ### batch HGT processing
        hsg_embeds = self.global_encoder.batch_HGTprocessing(batch_obj_embeds, batch_room_embeds, graph_info, batch_graph_info_dict)
        #### old version for HGT
        #### HGT processing for integrate the edge semantic information into object embeddings
        # hsg_embeds = self.HGT_processing(batch_obj_embeds, batch_room_embeds, graph_info)
        # cat visual embeddings
