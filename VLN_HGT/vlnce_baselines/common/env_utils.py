...
def get_places_room_inputs(config: Config):
    #Create a new dict
    places_dict = {}
    room_connection_dict = {}
    Base_dir = "/home/lg1/lujia/VLN_HGT/PlacesAndRoom/"
    dataset = make_dataset(config.TASK_CONFIG.DATASET.TYPE)
    print("dataset type: ", config.TASK_CONFIG.DATASET.TYPE)
    ...
    for scan_id in scenes:
        data_name = "/" + scan_id + ".pkl"
        with open(Base_dir + scan_id + data_name, 'rb') as f:
            loaded_data = pickle.load(f)
        places_info = loaded_data['places']
        loaded_room_connection = loaded_data['room_connection']
        room_connection_info = dict_to_adjacency_matrix(loaded_room_connection)
        places_dict[scan_id] = places_info
        room_connection_dict[scan_id] = room_connection_info
    return places_dict, room_connection_dict

def dict_to_adjacency_matrix(room_connection):
    if len(room_connection) == 0:
        n = 1
        adjacency_matrix = np.zeros((n, n), dtype=int)
        return adjacency_matrix
    else:
        n = max(room_connection.keys()) + 1
        adjacency_matrix = np.zeros((n, n), dtype=int)
        for room, neighbors in room_connection.items():
            for neighbor in neighbors:
                adjacency_matrix[room][neighbor] = 1
    return adjacency_matrix
    ...
    for i in range(num_gpus):
        for j in range(num_envs_per_gpu):
            ...
            if len(scenes) > 0:
                ...
                print("TASK CONFIG CONTENT SCENES: ", task_config.DATASET.CONTENT_SCENES)

            task_config.SIMULATOR.HABITAT_SIM_V0.GPU_DEVICE_ID = gpus[i]

            task_config.SIMULATOR.AGENT_0.SENSORS = config.SENSORS
            print("proc_config content scenes: ", proc_config.TASK_CONFIG.DATASET.CONTENT_SCENES)
            proc_config.freeze()
