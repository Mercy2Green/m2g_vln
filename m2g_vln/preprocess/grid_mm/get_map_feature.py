...
def build_feature_extractor(checkpoint_file=None):
    ...
    model, _, clip_preprocess = open_clip.create_model_and_transforms("ViT-B-16")
    model.input_resolution = 224
    model.patch_size = clip_config.patches_size
    model.width = clip_config.hidden_size
    model.layers = clip_config.transformer_num_layers
    model.heads = clip_config.transformer_num_heads
    model.to(device)
    model.eval()
...
def process_features(proc_id, out_queue, scanvp_list, args):
    ...
    for scan_id, viewpoint_id in scanvp_list:
        ...
        for k in range(0, len(images), args.batch_size):
            ...
            # print(b_fts[0].shape)
            b_fts = b_fts[0].data.cpu().numpy().astype(np.float16)
            # print(b_fts)
            fts.append(b_fts)
